...
const OrderPage = () => {
  ...
  const openStatus = async (id) => {
    try {
      const { data } = await axios.get(
        backendUrl + `/api/user/get-order-status/${id}`,
      );
      if (data.success) {
  ...
  const getOrder = async () => {
    try {
      const { data } = await axios.get(backendUrl + "/api/user/get-order", {
        params: { userId },
      });
          ...
          {orders.length === 0 ? (
            ...
            <div className=" h-[80vh] overflow-scroll py-6 scroll-hide flex flex-col gap-4">
              {orders.map((item, index) => (
                <div
                  key={index}
                  className="border-1 font-sans text-gray-700 border-gray-300 rounded-lg px-6 py-6"
                >
                  <div className="md:flex text-lg justify-between">
                    <span className="flex gap-2">
                      OrderId : <p>{item._id}</p>
                    </span>
                    <span className="flex gap-2">
                      Payment : <p>{item.paymentType}</p>
                    </span>
                    <span className="flex gap-2">
                      Total amount: <p>{item.amount}</p>
                    </span>
                  </div>
                  {item.items
                    ?.slice()
                    ...
                    .map((subItem, subIndex) => (
                      <div
                        className="md:flex items-center justify-between my-2"
                        key={subIndex}
                      >
                        <div>
                          <img src={subItem.image} className="w-22" alt="" />
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <span className="flex items-center gap-x-2">
                            Status:{" "}
                            <p
                              className={`text-lg ${item.orderStatus === "Cancelled" ? "text-red-700" : "text-green-700"}`}
                            >
                              {item.orderStatus}
                            </p>
                          </span>
                  ...
                  >
                    ...
                    <RxCross2
                      ...
                      className="cursor-pointer absolute right-3 top-3 rounded-full p-2 hover:bg-gray-200 text-[35px]"
                    />
                  </Model>
