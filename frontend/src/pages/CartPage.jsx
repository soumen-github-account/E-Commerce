...
const CartPage = () => {
  ...
  const { cartItems, fetchCart, removeFromCart, totalAmount, totalItems } =
    ...
    <div>
      ...
      <section className="w-full md:px-40 md:py-4 mt-6 md:flex bg-gray-100 grid grid-cols-1">
                ...
                <div className="mx-7 my-5 cursor-pointer" key={index}>
                  <div className="flex gap-3 hover:bg-gray-100">
                    ...
                    <div className="font-sans">
                      <p className="md:text-xl text-sm font-medium line-clamp-1">
                        {item.name}
                      </p>
                  ...
                  <div className="w-full flex">
                    <button
                      ...
                      className="px-3 my-1 py-1 text-sm rounded-sm text-white bg-red-500"
                    >
                      Remove
                    </button>
                  </div>
        ...
        <div className="md:ml-8 mt-4 h-[250px] bg-white rounded-sm px-3 py-3 shadow-md">
          ...
          <span className="font-sans flex justify-between">
            Price ({totalItems} items){" "}
            <p className="">₹ {cartItems.totalAmount}</p>
          </span>
          <span className="font-sans text-md flex justify-between">
            Delevary Charge <p>Free</p>
          </span>
          <hr className="my-2 text-gray-400" />
          <p className="flex justify-between text-md font-sans gap-8">
            Total Ammount : <span>₹ {cartItems.totalAmount}</span>
          </p>
          <hr className="my-2 text-gray-400" />
          {cartItems?.items?.length === 0 ? (
            ""
            ...
            <button
              className="bg-emerald-700 text-white rounded-sm px-4 py-1 cursor-pointer"
