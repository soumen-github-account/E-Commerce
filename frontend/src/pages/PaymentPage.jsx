...
const PaymentPage = () => {
  ...
  const orderhandler = async () => {
    try {
      setLoading(true);
      const { data } = await axios.post(backendUrl + "/api/user/order-placed", {
        userId,
        items,
        amount: total,
        address,
  ...
  return (
    <div className="flex w-full h-[100vh] bg-[#F1F0E9] items-center justify-center">
      <div
        className={`w-[90vw] mt-4 bg-[#F1F0E9] rounded-md ${orderSuccess ? "blur-sm" : ""}`}
      >
        ...
        <div className="bg-white my-1 shadow-sm py-2 px-2 font-sans">
          <div>
            <span
              ...
              onClick={() => setSelected("Online Payment")}
            >
            ...
            {selected === "Online Payment" && (
              <div className="w-full flex items-center justify-center my-4">
                <button
                  ...
                  className="bg-blue-800 rounded-sm py-1 px-20 text-white cursor-pointer"
                >
                  Pay {rupee} {total}
                </button>
              </div>
          ...
          <hr className="text-gray-400 border-dotted" />
          <span
            className="flex items-center justify-between py-1 px-3 my-2 cursor-pointer"
            onClick={() => setSelected("Cash On Delevery")}
          >
          ...
          {selected === "Cash On Delevery" && (
            <div className="w-full flex items-center justify-center my-4">
              {loading ? (
                ...
                <button
                  className="bg-emerald-800 rounded-sm py-1 px-20 text-white cursor-pointer"
        ...
        <div className="fixed inset-0 bg-transparent bg-opacity-40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg p-8 text-center animate-scale-in">
            ...
            <p className="text-gray-600">
              You will get your product to your ordered placed.
            </p>
          </div>
        </div>
      )}
