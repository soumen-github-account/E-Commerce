...
const CheckoutPage = () => {
  ...
  const { backendUrl, cartItems, totalAmount, totalItems, rupee } =
  ...
  const getAddress = async () => {
    try {
      const { data } = await axios.get(backendUrl + "/api/user/get-address", {
        params: { userId },
      });
      if (data.success) {
  ...
  const nextpage = () => {
    if (address.length === 0) {
      ...
      navigate("/payment-summary", {
        state: { total: lastTotal, items: items, address: selectedAddress },
      });
    }
  };
    ...
    <div className="flex flex-col items-center justify-center lg:px-90 md:mt-10">
      <div className="bg-[#F2F2F2] py-2 px-3 w-full rounded-md">
          ...
          items.map((item, index) => (
            <div className="my-5 cursor-pointer" key={index}>
              <div className="flex gap-3">
                ...
                <div>
                  <p className="md:text-xl text-md font-medium text-neutral-800 line-clamp-2">
                    {item.name}
                  </p>
                  <p className="text-md font-medium text-neutral-800">
                    size: {item.unit}
                  </p>
                  <p className="flex text-md font-medium text-neutral-800 font-sans">
                    qty: <p>{item.quantity}</p>
                  </p>
                  <span className="font-sans flex gap4 items-center">
                  ...
                  <p className="flex text-sm font-sans text-gray-700">
                    Delevery by: {item.deliveryDate}
                  </p>
                </div>
                <p className="text-sm text-green-600 mt-1 ml-9 hidden md:block">
                  free delevary
                </p>
              </div>
        ...
        <div>
          {selectedAddress && (
            <div>
              <p className="text-neutral-900 ml-1 mb-1 font-medium">
                Deliver to :
              </p>
              <div className="flex gap-3 font-sans items-center ml-1">
                <p>{selectedAddress.name}</p>
                <div className="px-2 py-[2px] rounded-sm bg-gray-50 border-1 border-gray-200 text-neutral-600 cursor-pointer">
              ...
              <div className="flex rounded-sm bg-white py-1 px-3 shadow-sm gap-1 justify-between items-center my-2">
                <div className="text-neutral-700">
                  <span className="flex gap-1">
                    <p>{selectedAddress.city}</p>,<p>{selectedAddress.state}</p>
                  </span>
                  <span className="flex gap-1">
                ...
                <button
                  ...
                  className="border-1 border-gray-300 rounded-sm py-1 px-2 text-emerald-600 font-medium cursor-pointer"
                >
                  Change
                </button>
              </div>
          ...
          {address.length === 0 ? (
            <div className="w-full flex items-center justify-center mb-2">
              <button
                ...
                className="py-2 bg-gray-800 rounded-md text-white px-6"
              >
                + Add Address
              </button>
            </div>
          ...
          <div className="bg-white rounded-lg my-2 py-3 px-4 shadow-sm border-1 border-gray-200">
            ...
            <div className="font-sans">
              ...
              <span className="flex items-center justify-between">
                <p className="text-gray-700">Delevery Chargs</p>
                <p className="text-green-800 gap-2 flex">
                  <span className="text-gray-400 line-through">{rupee}40</span>
                  Free
                </p>
              </span>
            </div>
            <hr className="my-2 text-gray-400 border-dotted gap-1" />
            <span className="flex items-center justify-between font-medium text-neutral-950 font-sans">
        ...
        <button
          ...
          className="bg-emerald-600 text-white px-4 py-2 cursor-pointer font-sans"
        >
          CONTINUE
        </button>
      </div>
        ...
        <div className="w-full bg-gray-50 h-[80vh] flex justify-center py-5">
          <div className="bg-white py-4 px-5 shadow-md min-h-[50vh] md:w-[40vw] w-full overflow-scroll scroll-hide">
              ...
              <div>
                {address.map((item, index) => (
                  <div
                    ...
                    className={`mt-4 cursor-pointer pl-2 text-neutral-700 relative font-sans flex flex-wrap gap-2 w-full rounded-lg py-4 ${selectedAddress._id === item._id ? "border-2 border-blue-700" : "border-1 border-gray-300"}`}
                  >
                    <p>{item.name},</p>
                    <p>{item.mobile},</p>
                    <p>{item.city},</p>
            ...
            <button
              ...
              className="w-full mt-3 py-3 cursor-pointer hover:bg-white hover:text-black border-1 border-black duration-300 bg-neutral-800 text-white rounded-sm text-lg font-medium"
            >
              Select
            </button>
          </div>
