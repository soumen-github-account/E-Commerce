        ...
        <div className="flex-none h-[80vh] border-r-1 border-r-gray-500 px-2 overflow-scroll scroll-hide">
          {catagoryData.map((item, index) => (
            <div
              ...
              className={`hover:bg-emerald-100 rounded-lg flex flex-col items-center my-1 py-1 w-18 md:w-20 cursor-pointer ${change === index ? "bg-emerald-100" : "bg-emerald-50"} duration-100`}
            >
              ...
              <div className="grid lg:grid-cols-9 md:grid-cols-7 grid-cols-3">
                {item.subCatagory2?.map((subItem, subIndex) => (
                  <div
                    ...
                    onClick={() =>
                      navigate(
                        `/all-produt/${item.subCatagory2[subIndex].name}`,
                      )
                    }
                  >
                    <img
                      src={subItem.img}
                      className="md:w-20 w-15 bg-gray-100 rounded-full cursor-pointer"
                      alt=""
                    />
                    <p className="text-[13px] md:text-sm text-gray-800">
                      {subItem.name}
                    </p>
                  </div>
                ))}
