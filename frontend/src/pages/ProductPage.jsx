...
const ProductPage = () => {
  const { id } = useParams();
  const { addToCart, allproduct, loading, setLoading, backendUrl } =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        backendUrl + `/api/product/review-add/${id}`,
          ...
          <div className="md:flex grid grid-cols-1 mt-4 md:px-20 bg-gray-50">
            ...
            <div className="flex flex-col items-start p-5">
              ...
              <div className="flex gap-1 text-yellow-500 md:text-[25px] text-[20px] mt-3 items-center">
                {[...Array(5)].map((_, i) =>
                  i < productInfo.averageRating ? (
                    <IoMdStar key={i} />
                  ) : (
                    <IoMdStarOutline key={i} />
                  ),
                )}
                <p className="text-blue-600 ml-3 text-sm font-serif">
                  Very Good <span className="font-serif">~ 23,456 Reviews</span>
                </p>
              </div>
              <div className="rounded-r-lg px-3 py-1 text-md bg-emerald-500 text-white mt-2">
                Top Discount of Sale
              </div>
              <span className="flex py-1 px-3 items-center text-lg font-sans">
                <div className="text-green-600 text-[20px] gap-1 flex items-center">
                  <FaArrowDown className="" /> {productInfo.discount}%{" "}
                </div>
                <p className="text-gray-500 ml-3 text-md line-through">
                  ₹ {originalPrice}
                </p>
                <p className="ml-3 text-xl">₹ {totalPrice}</p>
              </span>
              ...
              <div className="md:flex hidden flex-col mt-2">
                <p className="flex gap-3 items-center">
                  <BiSolidOffer className="text-[25px] text-green-600" />{" "}
                ...
                <div className="flex gap-15">
                  <div className="shadow-lg w-[15vw] h-[100px]">
                    ...
                    <div className="">
                      Upto ₹8.00 cashback as Amazon Pay Balance when you…
                    </div>
                  </div>
                  <div className="w-[15vw] h-[100px] shadow-lg">
                    <p className="font-medium text-lg">Bank Offer</p>
                    <div>
                      Upto ₹1,250.00 discount on select Credit Cards, HDFC…
                    </div>
                  </div>
                  <div className="w-[15vw] h-[100px] shadow-lg">
                    <p className="font-medium text-lg">Partner Offers</p>
                    <div>
                      3% off any 3, 5% off any 4 <span>View products</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex max-sm:w-[75vw] gap-4 font-sans overflow-scroll scroll-hide">
                  {productInfo.type.map((item, index) => (
                    <div
                      ...
                      key={index}
                      className={`px-6 cursor-pointer hover:bg-gray-200 py-2 rounded-sm ${selectedIndex === index ? "border-2 border-emerald-700" : "border-1 border-gray-300"}`}
                    >
                      {item}
                    </div>
                  ))}
              ...
              <div className="flex items-center gap-4 mt-3">
                <button
                  ...
                  className="flex items-center w-10 h-10 justify-center pb-[5px] rounded-full text-[25px] font-bold bg-emerald-600 text-white cursor-pointer"
                >
                  -
                </button>
                ...
                <button
                  ...
                  className="flex items-center w-10 h-10 justify-center rounded-full text-[25px] font-bold bg-emerald-600 text-white cursor-pointer"
                >
                  +
                </button>
              </div>
              <div className="flex mt-3 gap-4">
                {loading ? (
                  <button className="md:px-7 md:py-2 px-4 py-2 flex items-center text-white bg-emerald-600 rounded-full cursor-pointer md:text-[17px] text-md">
                  ...
                  <button
                    ...
                    className="md:px-7 md:py-2 px-4 py-2 text-white bg-emerald-600 rounded-full cursor-pointer md:text-[17px] text-md"
                  >
                    Add to cart
                  </button>
                )}
                {buyLoading ? (
                  ...
                  <button
                    ...
                    className="md:px-7 md:py-2 px-4 py-2 text-emerald-600 bg-emerald-50 border-2 border-emerald-800 rounded-full cursor-pointer md:text-[17px] text-sm font-bold"
                  >
                    Buy Now
                  </button>
                )}
              ...
              <div>
                <h1 className="text-lg font-medium my-2 underline">
                  Description
                </h1>
          ...
          <div className="w-full mt-20 md:px-6 px-2 pb-16">
            <div className="rounded-md border-1 border-gray-500 py-8 md:px-10 px-2">
              <div className="flex md:gap-6 gap-2">
                <button
                  onClick={() => setChangeButton("des")}
                  className="md:py-2 md:px-5 text-sm px-3 py-1 rounded-full border-2 border-emerald-500 text-emerald-600 hover:bg-emerald-50 cursor-pointer"
                >
                  Description
                </button>
                <button
                  onClick={() => setChangeButton("add")}
                  className="md:py-2 md:px-5 text-sm px-3 py-1 rounded-full border-2 border-emerald-500 text-emerald-600 hover:bg-emerald-50 cursor-pointer"
                >
                  Additional Info
                </button>
                <button
                  onClick={() => setChangeButton("review")}
                  className="md:py-2 md:px-5 text-sm px-3 py-1 rounded-full border-2 border-emerald-500 text-emerald-600 hover:bg-emerald-50 cursor-pointer"
                >
                  Reviews{" "}
                  ...
                  <table class="w-full text-sm text-left text-gray-700">
                    <tbody>
                      {productInfo.details.map((item, index) => (
                        <tr
                          class="border-b border-1 border-gray-500"
                          key={index}
                        >
                          <td class="p-3 font-medium bg-gray-100 w-1/2 text-md text-neutral-800">
                            {item}
                          </td>
                          <td class="p-3 border-l border-l-gray-500 text-md text-neutral-800">
                            {productInfo.details_type[index]}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              {changeButton === "review" && (
                <div className="mt-3">
                  <p className="font-medium text-2xl">
                    Customar questions and answers
                  </p>
                  <div className="w-full py-2 font-sans">
                    {productInfo.reviews && productInfo.reviews.length > 0 ? (
                      productInfo.reviews?.map((item, index) => (
                        <div
                          key={index}
                          className="flex items-center justify-between gap-1 rounded-md border-1 px-3 py-2 border-gray-400 flex-wrap"
                        >
                          <div className="flex flex-col items-center gap-2 ">
                            <img
                              src={item.userImage}
                              className="w-10 md:w-17 rounded-full"
                              alt=""
                            />
                            <p className="text-emerald-800 font-medium text-sm md:text-[19px]">
                              {item.name}
                            </p>
                          </div>
                  ...
                  <form onSubmit={handleSubmit} action="">
                    <div className="mt-3 flex flex-col md:w-[50vw] w-full">
                      ...
                      <textarea
                        ...
                        className="outline-none border-1 w-full border-gray-400 rounded-md px-3 py-3 mt-4"
                      />
                    </div>
                    ...
                    {loading ? (
                      ...
                      <button
                        type="submit"
                        className="bg-emerald-700 text-white cursor-pointer px-5 py-2 rounded-full mt-2"
                      >
                        Submit Review
                      </button>
                    )}
