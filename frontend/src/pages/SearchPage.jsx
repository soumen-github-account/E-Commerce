...
const SearchPage = () => {
    ...
    if (query.trim() !== '') {
        const filtered = allproduct.filter(product => {
            ...
            return (
                ...
                (Array.isArray(product.details) && product.details.some(d => d?.toLowerCase().includes(q)))
            );
        }).slice(0, 5);
  ...
  return (
    ...
    <div className='w-full lg:px-70 px-3 mt-6'>
        <div className='w-full border-1 border-gray-300 py-1 rounded-sm'>
            ...
            <div className='flex items-center px-2 ml-1 justify-between'>
                ...
                <IoSearch onClick={() => query.trim() && handleSelect(query)} className='cursor-pointer text-gray-500 hover:bg-gray-100 px-2 py-2 rounded-full text-[37px]'/>
            </div>
    ...
    {/* popular product */}
    <div className='px-3'>
        <h1 className='my-2 text-gray-700 font-medium text-[20px]'>Popular Products</h1>
        <div className='w-full grid md:grid-cols-8 grid-cols-3 gap-3'>
            {Array.isArray(randomProducts) && randomProducts.length > 0 ? (
            randomProducts.map((item, index) => (
                <div key={index} className='border-1 border-gray-400 rounded-sm py-2 px-2 items-center flex flex-col'>
